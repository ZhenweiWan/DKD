...
class KD_ours_strong(Distiller):
    def __init__(self, student, teacher, cfg):
        super(KD_ours_strong, self).__init__(student, teacher)
        self.temperature = cfg["KD"]["TEMPERATURE"]
        self.bt_loss_weight = cfg["KD"]["LOSS"]["BT_WEIGHT"]
        self.kd_loss_weight = cfg["KD"]["LOSS"]["KD_WEIGHT"]

    def forward_train(self, image_weak, image_strong, target, **kwargs):
        with torch.no_grad():
            tea_image_features_weak, tea_text_features_weak, logits_teacher_weak = self.teacher(image_weak)
            tea_image_features_strong, tea_text_features_strong, logits_teacher_strong = self.teacher(image_strong)

        image_ft_weak, logit_scale_weak = self.student(image_weak, target)

        logits_student_weak = logit_scale_weak * image_ft_weak @ tea_text_features_weak.t().detach()

        image_ft_strong, logit_scale_strong = self.student(image_strong, target)
        #
        logits_student_strong = logit_scale_strong * image_ft_strong @ tea_text_features_strong.t().detach()
        ...
        #     6.0,
        # )
        loss_bc_weak = self.bt_loss_weight * ((bc_loss(
            logits_student_weak,
            logits_teacher_weak,
            self.temperature,
        ) * mask).mean()) + self.bt_loss_weight * ((bc_loss(
            logits_student_weak,
            logits_teacher_weak,
            3.0,
        ) * mask).mean()) + self.bt_loss_weight * ((bc_loss(
