...
class DataManager:
    ...
    def __init__(
        ...
        self.dm_cfg = cfg
        self.dm_dataset_wrapper = dataset_wrapper

        if cfg.VERBOSE:
            self.show_dataset_summary(cfg)

    def reset_train_loader_x(self):
        self.train_loader_x = build_data_loader(
            self.dm_cfg,
            sampler_type=self.dm_cfg.DATALOADER.TRAIN_X.SAMPLER,
            data_source=self.dataset.train_x,
            batch_size=self.dm_cfg.DATALOADER.TRAIN_X.BATCH_SIZE,
            n_domain=self.dm_cfg.DATALOADER.TRAIN_X.N_DOMAIN,
            n_ins=self.dm_cfg.DATALOADER.TRAIN_X.N_INS,
            tfm=self.tfm_train,
            is_train=True,
            dataset_wrapper=self.dm_dataset_wrapper
        )
